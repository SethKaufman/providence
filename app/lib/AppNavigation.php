	...
	class AppNavigation extends BaseObject {
		...
		private function _genReverseNavTable() {
			...
			while(sizeof($va_stack) > 0) {
				...
				$vs_controller_path = is_array($va_action_info) ? '/'.join('/', array($va_action_info['module'], $va_action_info['controller'])) : '/';
				$va_tmp = (is_array($va_action_info) && isset($va_action_info['action'])) ? explode('/', $va_action_info['action']) : [];
